...
$(document).ready(function () {
    ...
    $('#month, #year').on('change', function () {
        ...
        window.location.href = `/Reports/FinancialKPIs?month=${month}&year=${year}`;
    });
...
//Current Day First Revenue - Open
//Current Day Highest Revenue - High
//Previous Day Lowest Revenue - Low
//Current Day Last Revenue - Close

function WinLossChart() {

    //const formattedData = candleChartData.map(item => ({
    //    x: new Date(item.TheDate),
    //    y: [item.firstSale, item.highestSale, item.lowestSale, item.lastSale]
    //}));

    const formattedData = candleChartData.map(item => ({
        x: new Date(item.theDate),
        //y: [item.open == 0 ? 1 : item.open, item.high == 0 ? 1 : item.high, item.low == 0 ? 1 : item.low, item.close == 0 ? 1 : item.close]
    ...
    var options = {
        ...
        xaxis: {
            type: 'datetime',
            labels: {
                format: 'dd MMM', // force format like "01 Apr"
                datetimeUTC: false
            }
        },
        yaxis: {
    ...
    var chart = new ApexCharts(document.querySelector("#WinLossChart"), options);
    chart.render();
}

function WinLossChart1() {
    var options = {
        series: [{
            data: [{
                x: new Date(1538778600000),
                y: [6500, 6550, 6400, 7000]
            },
            {

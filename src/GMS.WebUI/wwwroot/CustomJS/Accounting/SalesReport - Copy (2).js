...
function calculateRevenueAndCollection(currentData, previousData) {
    ...
    const currentRevenue = sum(currentData.filter(x => x.balance != null).map(x => x.totalDueAmount));
    const currentCollection = sum(currentData.map(x => x.payments));
    const previousRevenue = sum(previousData.map(x => x.totalDueAmount));
    const previousCollection = sum(previousData.map(x => x.payments));
    //const currentBalance = CalculateBalances(currentData); //sum(currentData.map(x => x.balance));
    //const previousBalance = CalculateBalances(previousData);//sum(previousData.map(x => x.balance));

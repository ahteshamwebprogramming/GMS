...
function ListPartialView() {
    ...
    $.ajax({
        ...
        url: '/FeedbackAttributesMaster/ListPartialView',
...
function AddPartialView(Id = 0) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/FeedbackAttributesMaster/AddPartialView',
        data: JSON.stringify(inputDTO),
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
...
function AddFeedback() {
    ...
    if (!isValidForm("AddFeedback")) {
        return;
    }
    
    let AddForm = $("#AddFeedback").find("[dbCol]");
    ...
    BlockUI();
    $.ajax({
        url: '/FeedbackAttributesMaster/Save',
        data: dataVM,
        //dataType: "json",
        async: false,
        type: 'POST',
        ...
        success: function (response) {
            $("#addFeedbackPopup").find(".btn-close").click();
            if (response != null) {
...
function DeleteList(Id) {
    Swal.fire({ title: 'Are you sure?', text: "This will get deleted permanently!", icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', customClass: { confirmButton: 'btn btn-primary me-3', cancelButton: 'btn btn-label-secondary' }, buttonsStyling: false }).then(function (result) {
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: "/FeedbackAttributesMaster/Delete",

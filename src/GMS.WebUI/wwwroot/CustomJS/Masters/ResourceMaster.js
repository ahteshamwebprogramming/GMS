$(document).ready(function () {
    ...
    initDates();
    ResourceMasterListPartialView();
    //GuestsListPartialView();
});
function initDates() {
    //InitialiseMobileFields();
    //$("#AddResource").find("[name='MemberDetail.Dob']").datetimepicker({
    //    format: 'd-m-Y',
    //    timepicker: false,
    //});
    //$("#AddResource").find("[name='MemberDetail.DateOfArrival']").datetimepicker({
    //    format: 'd-m-Y H:i',
    //    timepicker: true,
    //});
    ...
    ////    step: 30
    ////});
    $("#AddResource").find("[name='StartTime']").datetimepicker({
        datepicker: false,
        format: 'H:i',
        step: 10 // Optional: Define minute intervals
    });
    $("#AddResource").find("[name='EndTime']").datetimepicker({
        datepicker: false,
        format: 'H:i',
        step: 10 // Optional: Define minute intervals
    });
    $("#AddResource").find("[name='Duration']").datetimepicker({
        datepicker: false,
        format: 'H:i',
        step: 10 // Optional: Define minute intervals
    });
    ...
    $("#AddResource").find("[name='StartTime']").change(function () {
        CalculateEndTime();
    });
    $("#AddResource").find("[name='Duration']").change(function () {
        CalculateEndTime();
    });
}

function CalculateEndTime() {
    let startTime = $("#AddResource").find("[name='StartTime']").val();
    let duration = $("#AddResource").find("[name='Duration']").val();

    if (!startTime || !duration) {
        // Handle invalid input (optional)
        $("#AddResource").find("[name='EndTime']").val("Invalid input");
        return;
    }
    ...
    $("#AddResource").find("[name='EndTime']").val(endMoment.format("HH:mm"));

    /*$("#AddResource").find("[name='EndTime']").val("00:10");*/
}


function ResourceMasterListPartialView() {

    BlockUI();
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/ResourceMaster/ResourceMasterListPartialView',
        //data: JSON.stringify(inputDTO),
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            UnblockUI();
            $('#div_ResourceMasterListPartial').html(data);
        },
...
function AddResourceMasterPartialView(Id = 0) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/ResourceMaster/AddResourceMasterPartialView',
        data: JSON.stringify(inputDTO),
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            ...
            $('#div_ResourceMasterAddPartial').html(data);
            $("#btnOpenAddResourceMasterPopup").click();
            initDates();
            //getOnLoadAddGuests();
        },
        error: function (result) {
...
function AddResource() {
    ...
    if (!isValidForm("AddResource")) {
        return;
    }
    //alert("Success");
    //return;

    let AddForm = $("#AddResource").find("[dbCol]");
    ...
    BlockUI();
    $.ajax({
        url: '/ResourceMaster/SaveResourceMaster',
        data: dataVM,
        //dataType: "json",
        async: false,
        type: 'POST',
        ...
        success: function (response) {
            $("#addResourceMasterPopup").find(".btn-close").click();
            if (response != null) {
...
function DeleteList(Id) {
    Swal.fire({ title: 'Are you sure?', text: "This will get deleted permanently!", icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, delete it!', customClass: { confirmButton: 'btn btn-primary me-3', cancelButton: 'btn btn-label-secondary' }, buttonsStyling: false }).then(function (result) {
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: "/ResourceMaster/DeleteResourceMaster",
...
function ManageStatus(Id, ActiveStatus) {
    Swal.fire({ title: 'Are you sure?', text: "This will change status!", icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, change it!', customClass: { confirmButton: 'btn btn-primary me-3', cancelButton: 'btn btn-label-secondary' }, buttonsStyling: false }).then(function (result) {
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: "/ResourceMaster/ManageResourceMasterStatus",
                contentType: 'application/json',
                data: JSON.stringify(inputDTO),
                success: function (data) {

...
function ListPartialView() {
    ...
    $.ajax({
        ...
        url: '/CategoryMaster/ListPartialView',
...
function AddPartialView(Id = 0) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/CategoryMaster/AddPartialView',
        data: JSON.stringify(inputDTO),
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            ...
            $("#btnOpenAddCategoryMasterPopup").click();
        },
        error: function (result) {
            UnblockUI();
            $erroralert("Transaction Failed!", result.responseText);
...
function AddCategoryMaster() {
    ...
    if (!isValidForm("AddCategoryMaster")) {
        return;
    }

    let AddForm = $("#AddCategoryMaster").find("[dbCol]");
    ...
    $.ajax({
        ...
        success: function (response) {
            $("#addCategoryMasterPopup").find(".btn-close").click();
            if (response != null) {
...
function ManageStatus(Id, ActiveStatus) {
    Swal.fire({ title: 'Are you sure?', text: "This will change status!", icon: 'warning', showCancelButton: true, confirmButtonText: 'Yes, change it!', customClass: { confirmButton: 'btn btn-primary me-3', cancelButton: 'btn btn-label-secondary' }, buttonsStyling: false }).then(function (result) {
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: "/CategoryMaster/ManageCategoryMasterStatus",
                contentType: 'application/json',
                data: JSON.stringify(inputDTO),
                success: function (data) {

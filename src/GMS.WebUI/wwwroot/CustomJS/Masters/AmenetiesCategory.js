...
function ListPartialView() {
    ...
    $.ajax({
        ...
        url: '/AmenetiesCategory/ListPartialView',
...
function AddPartialView(Id = 0) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json; charset=utf-8",
        url: '/AmenetiesCategory/AddPartialView',
        data: JSON.stringify(inputDTO),
        cache: false,
        dataType: "html",
        success: function (data, textStatus, jqXHR) {
            ...
            $("#btnOpenAddAmenetiesCategoryPopup").click();            
        },
        error: function (result) {
            UnblockUI();
            $erroralert("Transaction Failed!", result.responseText);
...
function AddAmenetiesCategory() {

    if (!isValidForm("AddAmenetiesCategory")) {
        return;
    }
    
    let AddForm = $("#AddAmenetiesCategory").find("[dbCol]");
    ...
    $.ajax({
        ...
        success: function (response) {
            $("#addAmenetiesCategoryPopup").find(".btn-close").click();
            if (response != null) {
